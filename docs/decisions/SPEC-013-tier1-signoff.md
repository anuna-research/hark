...
| **J-a** | Fix the J-3 `ct-equal?` strict-`and` defect (wrong-length peer MAC must return `mac_mismatch` and bump the failed-attempt counter, not crash the handler — the N=3 deletion is a load-bearing REQ-007 control). | **DONE 2026-06-11** — cbcl-bus `feat/spec-016-pairing` 145cf95: `ct-equal?` now short-circuits via `if`; regression-tested (wrong-length mac_a/mac_b → `mac_mismatch`, no crash). |
| **J-b** | Negative-test suite from the audit §6: K-=-identity abort, wrong-length MAC (no crash + counter bump), M/N known-answer pin. | **DONE 2026-06-11** — cbcl-bus 145cf95: scalarmult-onto-identity rejected, malformed (non-canonical/truncated/oversized) points → `invalid_msg`, M/N known-answer pinned. The end-to-end counter-bump assertion through the ws-handler rides with the pairing driver. |
