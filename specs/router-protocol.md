...
5. stores the connection under the local handle
...
3. resolve `CBCL_AGENT_HANDLE`
4. send the frame through the daemon over that handle's WebSocket

Terminal messages should be CBCL `reply` or `error` messages. Progress messages
should be CBCL `tell` messages using the router's expected progress convention.
