...
> **REQ-025/REQ-026 status (v0.9.2):** normative-DRAFT, folding TWO fresh-context Principle-12
> reviews (findings F1–F8 then re-review #1–#5, `docs/decisions/SPEC-013-resync-review-findings.md`).
> Each obligation is a lettered clause so a [[#TEST-025]]/[[#TEST-026]] failure attributes to one
> clause (atomicity). Named constants: `FORK_THRESHOLD = 3` consecutive decrypt failures,
> `RESYNC_CAP = 3` re-request attempts, `RESYNC_WAIT_MS` the bounded re-request interval, and
...
>   and upgrade path.
> - [[#REQ-026]] — the **creator** half — is implemented EXCEPT clause (c), and the exception is
>   a design gap rather than unfinished work. See [[#BUG-002]]. What is implemented and verified:
>   (a) signature under the requester's **pinned** key plus a durable, strictly-monotonic nonce
>   floor per requester; (b) the creator ∧ elected-owner precondition; (d) routine re-announces and
>   non-member requests change nothing; (e) `RESYNC_RATE = 2` per requester per wall-clock hour,
>   durable across restarts.
>
>   **Corrections to the previous revision of this note, which overstated coverage.** A
>   fresh-context adversarial review established, by mutation, that three claims here were false:
>   the end-to-end test cited for clause (c) asserted nothing (`MlsSession::open` reloads the
>   persisted group, so its `group.is_some()` assertion held before the Welcome, which was in fact
>   rejected as a REQ-012c replacement); and the checks for (b) and (d) could each be deleted with
>   the whole suite still green. (d) now has a test using a **pinned** non-member, which the
>   previous one did not — it was refused at the pin lookup and never reached the liveness check it
>   named. (b) is **jointly** covered and cannot be covered per-check: `elect_committer` prefers the
>   genesis creator whenever the creator is a live leaf, so the creator check implies the owner
>   check and the two mask each other. The owner check is retained as defence against those
>   elections diverging again, and this specification should not claim it is separately verified.
>
>   `RESYNC_RATE` was not pinned by this specification; 2 per hour is chosen here and recorded so
>   it can be argued with.

### BUG-002 — a re-provisioner cannot tell which of a handle's KeyPackages it can open

**Severity:** S1 · **Priority:** P1 · **Status:** confirmed, mitigated by disabling the eviction.

[[#REQ-026]](c) has the re-provisioner fetch a fresh [[KeyPackage]] and validate it before removing
the stale leaf. The only checks available on that package are its credential handle and its leaf
signature key — and because a handle's MLS identity is bound to its **one** wire key ([[#REQ-007]]),
every package that handle has ever published, in every room, carries the same signature key.

The KeyPackage directory is per **handle** and global (`keypub`/`keyget` are addressed to `@hub` and
name no room), while providers — and therefore init private keys — are per **(handle, room)**. So a
package generated by another room's session is pin-valid here and unopenable here, and nothing
available to the validator distinguishes it.

The hub answers the `keyget`. It can therefore serve a pin-valid package whose private half this
room's peer does not hold: the Remove commits, the re-add produces a leaf nobody can use, and the
member is evicted into a [[Welcome]] it cannot open — with the roster still listing it, so no
ordinary Add re-adds it. That is [[SPEC-061-external-commit-admission#NFR-001]] violated: a hub
behaviour deciding who is in a group.

The **ordinary Add path has the same exposure and always has**; there it costs a failed Add that is
retried. Honouring a resync converts it into a committed eviction, which is the difference between
a nuisance and a member losing a channel.

*Mitigation, not resolution.* `REPROVISIONING_MAY_EVICT` is `false`: a resync from a live leaf is
verified, counted, recorded and then declined. Everything [[#REQ-026]] asks for is implemented
except the eviction itself.

*Proposed resolution, requiring a wire change and therefore cross-stack agreement.* Have the
requester name the [[KeyPackage]] it can open — its ref, inside the [[#REQ-025]](b) signed context —
so a hub substituting a different one is refused. The requester is the only party that knows which
packages it holds init keys for, which is where the check belongs. The ordinary Add path should be
reconsidered on the same grounds.

*Discovered by:* fresh-context adversarial review, with an executed probe.
...
- **REQ-025 — Fork recovery: desync self-heal (member side).** When the [[#REQ-006]]
  ...
  - **(b) Request re-admission with an *authenticated, replay-resistant* resync request.** Re-issue
    the readiness announce marked `:resync`, **signed** over `(label, from, key, room, nonce)` under
    the dedicated `cbcl-mls-resync/v1` label — reusing the [[#REQ-019]] self-signed-assertion
    machinery. The `nonce` SHALL be **strictly monotonic per requester** (e.g. a wall-clock
    timestamp), NOT the pin epoch — the pin epoch is constant absent a rotation, so a captured
    resync could be replayed to force an eviction (re-review #1). (The verifier's obligations —
    signature check and freshness — live in [[#REQ-026]](a).)
  - **(c) Bound and surface.** After sending a resync, if no valid re-admission Welcome arrives
    within `RESYNC_WAIT_MS`, the agent MAY re-request up to `RESYNC_CAP` times, then SHALL surface the
    desync as **terminal** (an operator-visible recovery failure). The retry-then-terminal path SHALL
    be reachable while groupless (review F4 — the shipped web code's terminal branch was unreachable
...
- **REQ-026 — Authorised re-provisioning (creator side).** On an **authenticated** resync request
  ...
  - **(a) Verify first (signature AND freshness).** SHALL, **before** minting any [[#REQ-014]]
    evidence: (i) verify the resync signature under the requester's **pinned** wire key (never a
    wire-supplied key) over the `cbcl-mls-resync/v1` context bound to this room — rejecting a forged
    or wrong-room request (review F1); AND (ii) reject a **replayed** request by requiring its nonce
    to be **strictly greater than the last nonce honored for that handle** (re-review #1 — a signed
    resync is otherwise capturable and replayable to evict a healthy member). An unsigned,
    badly-signed, stale-nonce, or unpinned resync SHALL NOT be honored.
  ...
  - **(c) Validate-then-remove-then-add (no un-rolled-back eviction).** SHALL obtain and
    ...
    a completable re-add). *Residual (re-review #3):* the pin pre-check is not a full dry-run of the
    MLS Add, so a Remove that merges (and fans) followed by an Add that fails for an unforeseen
    MLS-level reason CANNOT be rolled back (the Remove is already committed); the backstop is the
    victim's own [[#REQ-025]](c) bounded retry/terminal, and the re-provisioner SHOULD combine
    remove+add into a single Commit where the stack supports it.
  ...
  - **(e) Rate-limit.** SHALL bound honored resyncs to `RESYNC_RATE` per (member, **wall-clock
    window**) — NOT per MLS epoch: each honored resync itself bumps the epoch (a remove + an add
    Commit), so an epoch-keyed window resets on its own trigger and the bound is vacuous (re-review
    #2). The window SHALL be a fixed time bucket (or an epoch-independent monotonic counter) so the
    cap actually bounds forced-fork [[KeyPackage]] drain and churn (review F2).
...
- **OQ-006 — Resync abuse (forged eviction + forced-fork FS drain) — PARTLY RESOLVED in-spec; residual OPEN.**
  ...
  - **Forged AND replayed eviction (was mis-scoped as "availability-only churn"; review F1 + re-review #1) — RESOLVED by design.**
    ...
    committer to evict" attack [[#REQ-014]] declares closed, re-opened through the resync door.
    Signing alone was insufficient: the re-review found the hub could also **capture a genuine signed
    resync and replay it** (the nonce was the constant pin epoch). Both are **closed** by
    [[#REQ-025]](b) (signed, **strictly-monotonic** nonce) + [[#REQ-026]](a) (verify signature under
    the pinned key **and** reject a nonce ≤ the last honored) — SHALLs, not the SHOULD the draft used.
    No attacker key is ever seated (the re-add is [[#REQ-008]]/[[#REQ-011]] pin-validated).
...
- **Fork recovery (TEST-025, TEST-026)** — the [[#REQ-025]]/[[#REQ-026]] resync protocol, per the
  ...
  - **TEST-026** (creator). *Positive:* the creator validates a fresh pinned KeyPackage, then
    ...
    **before** any Remove is minted — no eviction. *Negative-input (re-review #1):* a **replayed**
    genuine resync (a captured frame re-sent) is rejected by the stale-nonce check — no **second**
    Remove is driven (the shipped cbcl-bus integration test S6 captures-and-replays and asserts
    zero Removes). *Negative-output (F3):* an elected owner that
