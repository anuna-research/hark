...
impl ChatIdentity {
    ...
    /// on first use. The 32-byte seed is stored raw, `0600` on Unix. A new key
    /// means a new identity (the hub TOFU-enrols it on the next `hello`).
    pub fn load_or_create(path: &Path) -> std::io::Result<Self> {
        if let Ok(bytes) = fs::read(path) {
            if let Ok(seed) = <[u8; 32]>::try_from(bytes.as_slice()) {
                return Ok(Self::from_seed(seed));
            }
            // Wrong-sized key file: refuse to silently overwrite an identity.
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidData,
                format!("identity file {} is not a 32-byte Ed25519 seed", path.display()),
            ));
        }
...
mod tests {
    ...
    fn signs_verifiably() {
        ...
        assert!(vk.verify(b"tampered", &Signature::from_bytes(&sig)).is_err());
    }
